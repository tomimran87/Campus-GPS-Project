...
class ScaledSigmoid(nn.Module):
    """
    Sigmoid scaled to a specific range [min_val, max_val].
    Useful for GPS because standard sigmoid (0,1) struggles to predict 
    exact boundaries like 0.0 or 1.0.
    """
...
class GeM(nn.Module):
    ...
    Learns to focus on salient features (like buildings) vs background.
    """
...
class EfficientNetGPS(BaseGPSModel):
    ...
    def _get_head(self, input_features):
        ...
        head = nn.Sequential(
            ...
            ScaledSigmoid(0.0, 1.0)          # Sigmoid scaled to [0,1]
        )
...
class EfficientNetGPS_withGEM(nn.Module):
    def __init__(self):
        super().__init__()
        # 1. Backbone
        self.backbone, num_features = self._get_backbone()
        
        # 2. Pooling (GeM)
        self.pool = GeM(p=3)
        
        # 3. Head
        self.head = self._get_head(num_features)

    def _get_backbone(self):
        effnet = models.efficientnet_b0(weights='DEFAULT')
        # Return features (MBConv blocks) and channel count (1280)
    ...
    def _get_head(self, input_features):
        return nn.Sequential(
            ...
            nn.Dropout(0.2),            
            nn.Linear(256, 2),
            ScaledSigmoid(min_val=-0.05, max_val=1.05) 
        )
