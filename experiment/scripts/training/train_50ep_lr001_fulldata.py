#!/usr/bin/env python3
"""
Experiment: 50 Epochs, LR=0.001, Full Data
==========================================

Training all 3 models with:
- 50 epochs (more training time)
- LR=0.001 (same as best EfficientNet experiment)
- Full data (100%)
- Standard 70/15/15 train/val/test split

Output: testing/50ep_lr001_fulldata/
"""
...
CONFIG = {
    ...
    "OUTPUT_DIR": Path("testing/50ep_lr001_fulldata"),
}
...
def write_experiment_log(data_info, all_results):
    ...
    with open(log_path, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("EXPERIMENT LOG: 50 Epochs, LR=0.001, Full Data\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"  Data: 100% ({data_info['n_total']} images)\n")
        f.write(f"  Split: Train={data_info['n_train']}, Val={data_info['n_val']}, Test={data_info['n_test']}\n")
        ...
        f.write(f"{'Model':<15} {'Mean(m)':<10} {'Median(m)':<10} {'<10m':<10} {'<20m':<10} {'Epochs':<10}\n")
        f.write("-" * 65 + "\n")
        
        for model, res in all_results.items():
            f.write(f"{model:<15} {res['mean_error']:<10.2f} {res['median_error']:<10.2f} "
                    f"{res['within_10m']:<10.1f} {res['within_20m']:<10.1f} {res['final_epoch']:<10}\n")
...
def main():
    ...
    print("=" * 70)
    print("EXPERIMENT: 50 Epochs, LR=0.001, Full Data")
    print("=" * 70)
    ...
    with open(CONFIG["OUTPUT_DIR"] / "combined_results.json", 'w') as f:
        json.dump({"experiment": "50ep_lr001_fulldata", "results": all_results}, f, indent=2)
    
    print("\n" + "=" * 70)
    print("TRAINING COMPLETE!")
    print("=" * 70)
    print(f"\nResults Summary:")
    print(f"{'Model':<15} {'Mean Error':<12} {'Within 10m':<12} {'Epochs Used'}")
    print("-" * 50)
    for model, res in all_results.items():
        print(f"{model:<15} {res['mean_error']:.2f}m{'':<6} {res['within_10m']:.1f}%{'':<6} {res['final_epoch']}")
