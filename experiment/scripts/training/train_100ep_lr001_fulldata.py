#!/usr/bin/env python3
"""
Experiment: 100 Epochs, LR=0.001, Full Data
=============================================

Training all 3 models with:
- 100 epochs (extended training)
- LR=0.001 (optimal for EfficientNet/ResNet18)
- Full data (100%)
- Standard 70/15/15 train/val/test split

Output: testing/100ep_lr001_fulldata/
"""
...
CONFIG = {
    ...
    "OUTPUT_DIR": Path("testing/100ep_lr001_fulldata"),
}
...
def load_and_split_data():
    """Load full data and split into train/val/test."""
    print("\n" + "=" * 60)
    print("DATA LOADING")
    print("=" * 60)
...
def create_experiment_log(all_results, data_info):
    ...
    with open(log_path, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("EXPERIMENT: 100 Epochs, LR=0.001, Full Data\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"  Scheduler: ReduceLROnPlateau (patience=7, factor=0.5)\n")
        f.write(f"  Device: {CONFIG['DEVICE']}\n\n")
        
        f.write("DATA SPLIT:\n")
        f.write("-" * 40 + "\n")
        f.write(f"  Total: {data_info['n_total']} images\n")
        f.write(f"  Train: {data_info['n_train']} (70%)\n")
        f.write(f"  Val: {data_info['n_val']} (15%)\n")
        f.write(f"  Test: {data_info['n_test']} (15%)\n\n")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("EXPERIMENT: 100 Epochs, LR=0.001, Full Data")
    print("=" * 70)
