#!/usr/bin/env python3
"""
Experiment: 100 Epochs, LR=0.0001, Full Data
=============================================

Final experiment to test if ConvNeXt performs best with:
- Full dataset (100% of data)
- 100 epochs
- Learning rate: 0.0001 (optimal for ConvNeXt)
...
CONFIG = {
    ...
    "OUTPUT_DIR": Path("testing/100ep_lr0001_fulldata"),
}
...
def load_and_split_data():
    """
    Load full data and split into train/val/test (70/15/15).
    """
    print("\n" + "=" * 60)
    ...
    print(f"\n📌 EXPERIMENT DESIGN:")
    print(f"   Full data: {n_total} images")
    print(f"   Training set: {len(train_idx)} images ({CONFIG['TRAIN_SPLIT']*100:.0f}%)")
    print(f"   Validation set: {len(val_idx)} images ({CONFIG['VAL_SPLIT']*100:.0f}%)")
    print(f"   Test set: {len(test_idx)} images ({CONFIG['TEST_SPLIT']*100:.0f}%)")
    ...
    # Normalize GPS using training set only
    ...
    X_test_proc = np.transpose(X_test, (0, 3, 1, 2)).astype(np.float32) / 255.0
...
def train_model(model_name, model_class, train_loader, val_loader, test_loader, min_val, max_val):
    ...
    # Save model
    model_path = output_dir / f"{model_name}_100ep_fulldata.pth"
...
def main():
    print("=" * 70)
    print("EXPERIMENT: 100 Epochs, LR=0.0001, Full Data")
    print("=" * 70)
    ...
    with open(CONFIG["OUTPUT_DIR"] / "experiment_summary.json", 'w') as f:
        json.dump({
            'experiment': '100ep_lr0001_fulldata',
            'config': {
                'epochs': CONFIG['EPOCHS'],
