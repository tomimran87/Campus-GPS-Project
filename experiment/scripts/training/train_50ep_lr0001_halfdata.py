#!/usr/bin/env python3
"""
Experiment: 50 Epochs, LR=0.0001, Half Data
============================================

Training all 3 models with:
- 50 epochs (more training time)
- LR=0.0001 (lower LR - better for ConvNeXt)
- Half data (50%) for training
- Standard 70/15/15 train/val/test split from the 50%

Output: testing/50ep_lr0001_halfdata/
"""
...
CONFIG = {
    ...
    "PATIENCE": 10,
    "SEED": 42,
    "DATA_FRACTION": 0.5,  # Use half the data
    "DEVICE": torch.device("cuda" if torch.cuda.is_available() else "cpu"),
    "OUTPUT_DIR": Path("testing/50ep_lr0001_halfdata"),
}
...
def load_and_split_data():
    """Load half of data and split into train/val/test."""
    print("\n" + "=" * 60)
    print("DATA LOADING")
    print("=" * 60)
    ...
    # Take first half
    n_use = int(n_full * CONFIG["DATA_FRACTION"])
    X = X[:n_use]
    y = y[:n_use]
    
    n_total = len(X)
    print(f"Using {CONFIG['DATA_FRACTION']*100:.0f}%: {n_total} images")
...
def train_model(model_name, model_class, train_loader, val_loader, test_loader, min_val, max_val):
    ...
    # Save model
    model_path = output_dir / f"{model_name}_50ep.pth"
...
def create_experiment_log(all_results, data_info):
    """Create summary log file."""
    log_path = CONFIG["OUTPUT_DIR"] / "experiment_log.txt"
    
    with open(log_path, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("EXPERIMENT: 50 Epochs, LR=0.0001, Half Data\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"  Data Fraction: {CONFIG['DATA_FRACTION']*100:.0f}%\n")
        ...
        f.write("DATA SPLIT:\n")
        f.write("-" * 40 + "\n")
        f.write(f"  Full Dataset: {data_info['n_full']} images\n")
        f.write(f"  Used: {data_info['n_used']} images ({data_info['data_fraction']*100:.0f}%)\n")
        f.write(f"  Train: {data_info['n_train']}\n")
        f.write(f"  Val: {data_info['n_val']}\n")
        f.write(f"  Test: {data_info['n_test']}\n\n")
        
        f.write("RESULTS:\n")
        f.write("-" * 40 + "\n")
        f.write(f"{'Model':<15} {'Mean(m)':<10} {'Median(m)':<12} {'<10m':<10} {'<20m':<10} {'Epochs':<8}\n")
        f.write("-" * 65 + "\n")
        
        for res in all_results:
            f.write(f"{res['model']:<15} {res['mean_error']:<10.2f} {res['median_error']:<12.2f} "
                   f"{res['within_10m']:<10.1f}% {res['within_20m']:<10.1f}% {res['final_epoch']:<8}\n")
        
        # Best model
        best = min(all_results, key=lambda x: x['mean_error'])
        f.write("\n" + "=" * 70 + "\n")
        f.write(f"🏆 BEST MODEL: {best['model']}\n")
        f.write(f"   Mean Error: {best['mean_error']:.2f}m\n")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("EXPERIMENT: 50 Epochs, LR=0.0001, Half Data")
    print("=" * 70)
    ...
    create_experiment_log(all_results, data_info)
    
    # Print final summary
    print("\n" + "=" * 70)
    print("FINAL RESULTS SUMMARY")
    print("=" * 70)
    print(f"{'Model':<15} {'Mean(m)':<10} {'Median(m)':<12} {'<10m':<10} {'<20m':<10}")
    print("-" * 60)
    
    for res in all_results:
        print(f"{res['model']:<15} {res['mean_error']:<10.2f} {res['median_error']:<12.2f} "
              f"{res['within_10m']:<10.1f}% {res['within_20m']:<10.1f}%")
    
    best = min(all_results, key=lambda x: x['mean_error'])
    print("\n" + "=" * 70)
    print(f"🏆 BEST: {best['model']} with {best['mean_error']:.2f}m mean error")
    print("=" * 70)
